...
class HomeViewController: UIViewController,FSPagerViewDataSource,FSPagerViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,SFSafariViewControllerDelegate{
...
//                let slideUrl:String = "\(self.delegate)/api/slider"
...
//        navigationController?.pushViewController(destination, animated: true)
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if collectionView == self.saleCollectionView{
            return saleGameArray.count
        }
       else if collectionView == self.rentCollectionView{
            return rentGameArray.count
...
//            UIApplication.shared.open(URL(string: "https://www.easyBazi.ir/")!, options: [:], completionHandler: nil)
            let safariVC = SFSafariViewController(url: URL(string: "https://www.varzesh3.com/")!)
            self.present(safariVC, animated: true, completion: nil)
            safariVC.delegate = self
        }
    }
