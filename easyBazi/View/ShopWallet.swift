...
//
//  Payment Comfirm ALert View.swift
//  Easy Bazi
//
...
class ShopWallet : NSObject , UITextViewDelegate,SFSafariViewControllerDelegate{
...
//        textView.widthAnchor.constraint(equalToConstant: containerView.frame.size.width * 8/10).isActive = true
    ...
    @objc  func sendInfoToServer(){
        ...
        Address.set(url, token: getToken(), cityId: cityId, stateId: stateId, postCode: postCode, phoneNumber: PhoneNumber, content: addressDetailes) { (address, status, message) in
            if status == 1{
                self.addressId = address.id
                self.shopVC.shopGameWithWallet(self.addressId)
