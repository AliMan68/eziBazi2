...
//  EziBazi2
//
//  Created by AliArabgary on 4/5/19.
...
class SearchRentCell: baseCell {
    var rentPercent:Int?
    var game:Game!{
        didSet{
             if game.game_info.photos.count != 0{
                let url = URL(string:game.game_info.photos[0].url)
                gameImage.sd_setImage(with:url, placeholderImage:UIImage(named:"logo"),completed: nil)
                }else{
            ...
            if game.count == 0{
                ...
                let price = Int(game.price)/100
                            var weeklyRentPrice:String = ""
                            if rentPercent != nil{
                            weeklyRentPrice = (price*rentPercent!).formattedWithSeparator
                            }else{
                                rentPercent = 10
                                weeklyRentPrice = (price*rentPercent!).formattedWithSeparator
                            }
                           
                           gamePrice.text = convertToPersian(inputStr: "\(String(describing: weeklyRentPrice))") + " تومان "
                       }
...
//    fileprivate func configerentperiod(){
//        addSubview(rentperiod)
//        rentperiod.topAnchor.constraint(equalTo: gameRegion.bottomAnchor, constant: 0).isActive = true
//        rentperiod.leftAnchor.constraint(equalTo: leftAnchor, constant: 8).isActive = true
//        rentperiod.rightAnchor.constraint(equalTo: gameImage.leftAnchor, constant: -4).isActive = true
//
//    }
    ...
    fileprivate func configeView(){
        ...
        //        priceContainer.topAnchor.constraint(equalTo: gameImage.bottomAnchor, constant: -8).isActive = true
