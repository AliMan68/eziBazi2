...
class BottomList: NSObject, UICollectionViewDataSource, UICollectionViewDelegate,
UICollectionViewDelegateFlowLayout {
    ...
    public func getCities(cityId id:Int){
        ...
        indicator.startAnimating()
        let delegate = (UIApplication.shared.delegate as! AppDelegate).url
          let url =  "\(delegate)/api/state/\(rentViewController.currentStateId!)/cities"
...
//                    self.states.removeAll()
    ...
    public func showList(){
        ...
        if let window = UIApplication.shared.keyWindow{
            ...
            UIView.animate(withDuration: 0.9, delay: 0, usingSpringWithDamping: 0.8, initialSpringVelocity: 1, options: .curveEaseInOut, animations: {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if isCitiesDataSource{
            rentViewController.city.text = cities[indexPath.row].name
            rentViewController.currentCityId = cities[indexPath.row].id
            
        }else{
            rentViewController.state.text = states[indexPath.row].name
            rentViewController.city.text = ""
            rentViewController.currentStateId = states[indexPath.row].id
            rentViewController.selectCityBtn.layer.opacity = 1
            rentViewController.selectCityBtn.layer.borderColor = UIColor.easyBaziTheme.cgColor
